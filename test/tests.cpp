...
TEST(DeanaryTest, test3) {
  Student student(1, "TestName");
  EXPECT_EQ(false, student.isleadOfGroup());
}
...
TEST(DeanaryTest, test8) {
  ...
  EXPECT_EQ(
      true,
      dean.getGroup("FundamentalMathematics")->getlead()->isleadOfGroup());
}
...
TEST(DeanaryTest, test10) {
  ...
  Deanary dean;
  dean.AddGroup(group);
  EXPECT_EQ(nullptr, dean.getGroup(" FundamentalMathematics")->getlead());
  EXPECT_EQ(nullptr, dean.getGroup("FundamentalMathematics")->getlead());
}