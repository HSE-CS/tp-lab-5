...
int main() {
  Deanary my_deanary;
  my_deanary.hireStudents("./studentsData.txt");
  my_deanary.createGroups("./groupsData.txt");
  ...
  std::cout << "head: " << test_group.getHead().getFio()
    << std::endl << std::endl;
  Student test_student = test_group.getStudent(0);
  std::cout << "Student (first in Group1) info (read only): "
    << std::endl;
  std::cout << "name: " << test_student.getFio() << std::endl;
  std::cout << "group: " << test_student.getGroup().getTitle()
    << std::endl << std::endl;

  std::cout << "FIRING!" << std::endl;
  my_deanary.fireStudent(3.0);
  my_deanary.getStatistics();
  my_deanary.saveStuff("./Saves/");
}
