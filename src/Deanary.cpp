...
void Deanary::createStudents(std::fstream &studentsStream) {
    ...
    while (std::getline(studentsStream, str)) {
        ...
        std::string number = str.substr(indexThirdSpace + 1, str.size() - indexThirdSpace - 3);
...
void Deanary::getStatistic() {
    for (auto gr : groups){
        std::cout << "Group " << gr.getGroupName() << " statistics: " << gr.groupMiddleMark() << "\n";
        for (auto st : gr.students){
            std::cout << "Student " << st.getFio() << " statistics: " << st.getMiddleMark() << "\n";
        }
    }
