...
void Deanary::createGroups(std::string StudentsFile, std::string GroupsFile) {
    ...
    for(int i = 0; i < this->number_of_groups; i++) {
        this->groups.push_back(this->hireStudents(StudentsFile,
                                                  new Group(read.groups[i].second, read.groups[i].first),
                                                  std::to_string(i + 1)));
    }
}
...
Group* Deanary::hireStudents(std::string StudentFile, Group *someGroup, std::string groupId) {
    Reader Students;
    Students.StudentsReader(StudentFile);
    for(int j = 0; j < Students.names.size(); j++){
        if(Students.names[j].first[0] == Students.ids[j].second[0] && Students.names[j].first[0] == groupId[0]){
            someGroup->addStudent(new Student(atoi(Students.ids[j].second.c_str()), Students.names[j].second));
        }
    }
    return someGroup;
}
...
void Deanary::getStatistics() {
    ...
    for (auto group : groups){
        for(auto student : group->students){
            std::cout<<student->fio <<" has average mark: " << student->getAveragemark() << std::endl;
        }
    }

    for(auto group : groups){
        std::cout<<group->title << " has average mark: " << group->getAverageMark() << std::endl;
    }
}
    ...
    if(DeanFile.is_open()){
        for(auto group : groups){
            ...
            for(auto student : group->students){
                ...
                DeanFile << std::to_string(student->getId())<<":"<<student->getFIO()<<" -"<<marks<<"\n";
            }
        }
    }
    DeanFile.close();
