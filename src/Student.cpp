...
Student::Student(const std::string name, const std::vector<int16_t>& marks)
    : name(name), marks(marks) {
    this->id = id_generator++;
}

Student::Student(const std::string name, const std::shared_ptr<Group>& group)
    : name(name), group(group) {
    this->id = id_generator++;
}

Student::Student(const std::string name, const std::shared_ptr<Group>& group,
    const std::vector<int16_t>& marks) :
    name(name), group(group), marks(marks) {
    this->id = id_generator++;
}
...
float Student::getAverageMark() const {
    return static_cast<float>( std::accumulate(this->marks.begin(),
        this->marks.end(), 0) ) / this->marks.size();
}
